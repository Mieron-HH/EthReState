...
router.post(
	...
	async (req: Request, res: Response) => {
		...
		try {
			...
			existingProperty.images.push(propertyImage);
			existingProperty.save();

			return res.status(201).send(existingProperty);
		} catch (error) {
			console.error(error);
			throw new BadRequestError("Error adding property image");
		}
	}
);

export { router as AddPropertyImageRouter };
